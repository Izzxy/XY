...
<div>
  ...
  <el-table :data="tableData" stripe>
    ...
    <el-table-column label="操作" width="260">
      <template v-slot="scope">
        <!-- scope.row 就是当前行数据-->
        <el-button type="warning" @click="handelAccountAdd(scope.row)">充值</el-button>
        ...
        >
          <el-button  type="danger" slot="reference" style="margin-left: 3px">删除</el-button>
        </el-popconfirm>
      </template>
    </el-table-column>
  </el-table>
  ...
  data() {
    const checkNums = (rule, value, callback) => {
      ...
      if (value < 10 || value > 200) {
        callback(new Error('请输入大于等于10小于或等于200整数'));
      }
      callback()
    };
    return {
